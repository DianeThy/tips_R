...
livre_comptable <- read_sheet("https://docs.google.com/spreadsheets/d/1rdjsLuY3WGDomlFm-61DgQ1qn2bNM_VHo1a0LV-WUD8/edit#gid=38666601")
# Attention : ne marche que si le tableau est en format GoogleSheet et pas Excel !!
```
...
```{r}
# Exemple de Lyon - données projets
comm1 <- propositions |> 
    filter(lengths(comments) != 0) |> 
    group_by(id_proposition) |> 
    mutate(nb_comments = nrow(comments[[1]])) |> 
    select(id_projet, id_proposition, comments, nb_comments) |> 
    pluck() |> bind_rows()
comm2 <- comm1 |>
    pull(comments) |> 
    pluck() |> bind_rows()
...
proj_comm_date <- cbind(comm1.1, comm2) |> ungroup() |> 
    select(id_projet, author_id, publishedAt) |> 
...
# Géométrie additionnelle
  ...
  geom_label(aes(y = 1, label = title_projet, hjust = "bottom", group = groupes), fontface = "italic", size = 2.6,
...
ggplot(data) +
    geom_point() +
    labs(title = "Décorrelation entre la phase <span style='color: #323465; font-size: 23pt;'>Analyse, traitement, calcul</span> et les autres phases du cycle de vie") +
...
***Attention bien mettre "self_contained: false" dans le header. Affiche error quand preview de l'HTML ; il faut ouvrir dans un browser pour voir le résultat !!***
...
***Mettre ça dans <style> en haut du rmd pour enlever la bordure noire.***
...
+ **Autmatic tab for elements of a list**

```{r}
#voir script ici : https://github.com/datactivist/plans_de_charge/blob/main/scripts/tableau_de_bord.Rmd
...
```{r}
git stash push scripts/tableau_de_bord.html
```
...
+ **Commit conflicts Git R**

```{r}
# annuler le dernier commit : git stash
# supprimer tous les changements locaux (pull possible ensuite) : git stash push --include-untracked
```
