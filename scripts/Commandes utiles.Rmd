...
```{r}
x <- data.frame(col1 = c("BASSIN DE POMPEY", "CC AURE ET LOURON", "CC DU BRIANCONNAIS"),
                col2 = c(2, 1, 7))
```
...
```{r}
data <- data |> mutate(col = removeWords(col, c("IEEE ", "ACM ", "SIAM ")))
```
...
```{r}
data <- data |> mutate(gsub("\\W*\\b\\w\\b\\W*", " ", col))
```
...
```{r}
data <- data |> mutate(first_word = word(booktitle, 1))
```
...
```{r}
data <- data |> mutate(sub_string = substr(data$chaine_charac, 1, n))
```

+ **Extract n last characters**

```{r}
data <- data |> mutate(sub_string = substr(data$chaine_charac, nchar(data$chaine_charac)-n+1, nchar(data$chaine_charac)))
```
...
data <- data |> mutate(nom = str_pad(nom, 14, pad = "0")) #obliger à avoir 14 caractères (donc ajoute 0 en début si besoin)
```

+ **Remove first digit if 0**

```{r}
data <- data |> mutate(nom = gsub("^0", "", nom))
```

+ **Only 1 character in a string**

```{r}
nchar(capital_word) == 1
```
...
```{r}
grepl("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}", vote)
```
...
grid.arrange(g1,g2,g3, ncol=3, nrow = 1, 
             top = grid::textGrob("Comment intervenez-vous sur ces activités ?", gp = grid::gpar(fontsize = 15, font = 2)))
# Aligner les boxes
library(cowplot)
...
saving_plot <- function(graph, name, width, height) {
  ggsave(file = glue("~/Downloads/dataviz_marg/SVG/{name}.svg"), plot=graph, width=width, height=height)
  ggsave(file = glue("~/Downloads/dataviz_marg/PNG/{name}.png"), plot=graph, width=width, height=height)
}
saving_plot(graph, "age_histogram", 9, 5)
```
