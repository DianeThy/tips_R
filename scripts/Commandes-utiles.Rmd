...
communes_contours_geo |> 
    ...
    mapview(zcol = "dep_name",
            ...
            col.regions = c("Corse-du-Sud" = "#b5dbfb", "Haute-Corse" = "#1d82df"),
            popup = popupTable(communes_contours_geo, zcol = c("dep_name"))) +
    mapview(st_as_sf(na.omit(data_graph), coords = c("Longitude", "Latitude")),
          layer.name = "Commerces répondants",
          col.regions = "red", 
          alpha.regions = 1, map.types = "CartoDB.Positron",
          cex = "n", 
          popup = data_graph$Nb_users, #texte quand on clique sur un point
          label = data_graph$Nb_users, #texte au survol de la souris
          legend = FALSE)
...
data |> 
  ...
  ggplot(aes(area = `COUNTA de Action`, fill = Importance, subgroup = `Aspect intéropérabilité`)) +
      geom_treemap(col = "white", size = 4, alpha = .6) +
      geom_treemap_text(aes(label = ifelse(`COUNTA de Action` == 1, 
                                           paste0(`COUNTA de Action`, " action"),#\n(n° , ex_num_action, ")"
                                           paste0(`COUNTA de Action`, " actions"))), #\n(n° , ex_num_action, ")"
                        place = "centre", grow=F) +
      #geom_treemap_subgroup_text(aes(label = `Aspect intéropérabilité`)) + 
...
comp_data |> 
  ...
  labs(title = "Longeur minimales, maximales et moyennes des iris", x = "Longeur en cm", y = "") +
